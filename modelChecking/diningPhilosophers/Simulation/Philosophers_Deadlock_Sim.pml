...
proctype phil(int idN) {
	...
	:: cycle < CYCLES -> atomic {
		...
		:: (Forks[idN] == idR) && (Forks[idL] == idL); 
			printf("%d: Philosopher (%d) is thinking with NO forks -> Cannot acquire fork\n", cycle, idN)
		...
		:: (Forks[idN] == idR) && (Forks[idL] == -1); 
			Forks[idL] = idN; 
			printf("%d: Philosopher (%d) is thinking NO forks -> Acquires fork (%d)\n", cycle, idN, idL)	
		...
		:: (Forks[idN] == -1) && (Forks[idL] == -1);			
			...
			if 
			:: Forks[idL] = idN; 
			printf("%d: Philosopher (%d) is thinking with NO forks -> Acquires fork (%d)\n", cycle, idN, idL)
		...
		:: (Forks[idN] == idN) && (Forks[idL] == idL);
			printf("%d: Philosopher (%d) is thinking with fork (%d) -> Cannot acquire fork\n", cycle, idN, idN)
		...
		:: (Forks[idN] == idN) && (Forks[idL] == -1); 
			Forks[idL] = idN; 
			printf("%d: Philosopher (%d) is thinking with fork (%d) -> Acquires fork (%d)\n", cycle, idN, idN, idL)	
		...
		:: (Forks[idN] == idN) && (Forks[idL] == idN); 
			Forks[idN] = -1;
			Forks[idL] = -1;
			printf("%d: Philosopher (%d) is eating with forks (%d, %d) -> Releases Forks (%d, %d)\n", cycle, idN, idN, idL, idN, idL)
			
		
		:: else; 
			printf("%d: Philosopher (%d) is blocking \n", cycle, idN);	
		fi 
